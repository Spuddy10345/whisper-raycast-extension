...
/**
 * Lowercase and strip sentence punctuation. Keeps ASCII apostrophes (for
 * contractions like "don't") and hyphens (for compound words like
 * "well-known"). Strips: . , ; : ! ? " ( ) [ ] { } and smart double quotes
 * and em/en dashes. Whisper produces straight apostrophes for contractions,
 * so this preserves word integrity while dropping the punctuation marks the
...
export default function NoPunctuationDictateCommand() {
  ...
  const { startTranscription, handlePasteAndCopy } = useTranscription({
    ...
    setErrorMessage,
    setTranscribedText,
    refineText: async (text: string) => text, // No AI refinement in this mode
    saveTranscriptionToHistory,
    cleanupAudioFile,
    aiErrorMessage: "",
    skipAIForSession: true,
    transformText: stripPunctuation, // The actual difference vs dictate-simple
  });
  ...
  const generateWaveformMarkdown = useCallback(() => {
    ...
    let waveform = "```\n";
    waveform += "RECORDING (no punctuation)... PRESS ENTER TO STOP\n\n";
        ...
        return (
          <ActionPanel>
            ...
            <Action
              ...
              icon={Icon.ArrowClockwise}
              onAction={() => {
                showHUD("Please reopen the Dictate (No Punctuation) command.");
                closeMainWindow({ clearRootSearch: true, popToRootType: PopToRootType.Immediate });
              }}
            />
            <Action
    ...
    <Form
      ...
      navigationTitle={
        state === "transcribing"
          ? "Transcribing..."
          : state === "done"
            ? "Transcription Result (No Punctuation)"
            : state === "error"
              ? "Error"
              : "Dictate (No Punctuation)"
      }
    >
      ...
      {(state === "done" || state === "transcribing" || state === "idle") && (
        <Form.TextArea
          id="dictatedText"
          title={state === "done" ? "Dictated Text (No Punctuation)" : ""}
          placeholder={
            state === "transcribing"
              ? "Transcribing audio..."
              : state === "done"
                ? "Transcription result (lowercased, punctuation stripped)"
      ...
      {state === "idle" && (
        <Form.Description text="Casual dictation: output is lowercased with sentence punctuation stripped (apostrophes and hyphens kept). Press Enter to start recording." />
      )}
    </Form>
  );
}
